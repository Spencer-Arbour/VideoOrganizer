...
class TestTvDbConnector:
    ...
    def test_proper_token_retrieved(self, monkeypatch):
        ...
        tv_db = _TvDbConnector("fake", "lie", "nope").retrieve_access_token()
    ...
    def test_authorization_error_when_retrieving_token(self, monkeypatch):
        ...
        with pytest.raises(requests.ConnectionError) as excinfo:
            _TvDbConnector("fake", "lie", "nope").retrieve_access_token()
    ...
    def test_get_returns_json(self, monkeypatch):
        ...
        tv_db = _TvDbConnector("fake", "lie", "nope")._get("fake_url")
    ...
    def test_authorization_error_when_retrieving_token(self, error_code, monkeypatch):
        ...
        with pytest.raises(requests.ConnectionError) as excinfo:
            _TvDbConnector("fake", "lie", "nope")._get("fake")
    ...
    def test_search_for_series_only_does_same_search_once(self, monkeypatch):
        call_counts = count()
        monkeypatch.setattr(_TvDbConnector, "_get", lambda x, url, params: str(next(call_counts)))

        tv_db = _TvDbConnector("fake", "lie", "nope")
    ...
    def test_search_for_episodes_only_does_same_search_once(self, monkeypatch):
        call_counts = count()
        monkeypatch.setattr(_TvDbConnector, "_get", lambda x, url: dict({"fake": next(call_counts)}))
        ...
        assert first_fake.get("fake") == second_fake.get("fake") == 0
        assert first_flam.get("fake") == 1
