...
class Options(SecondWindow):
    ...
    def __init__(self, parent, **kw):
        ...
        self._name = LabelEntryCombo(self._frame).setup_label("name")\
            .setup_entry(width=self._ENTRY_WIDTH)\
            .grid(row=0, column=0, columnspan=2)

        self._foo = LabelEntryCombo(self._frame).setup_label("foo")\
            .setup_entry(width=self._ENTRY_WIDTH)\
            .grid(row=1, column=0, columnspan=2)

        self._bar = LabelEntryCombo(self._frame).setup_label("bar")\
            .setup_entry(width=self._ENTRY_WIDTH)\
